...
class SimpleGraph:
    ...
    def _GetAdjacentIndexes(self, VertexIdx: int):
        VertexMapping = self.m_adjacency[VertexIdx]

        AdjacentIdx = []
        IsAdjacentSign = 1

        for Pointer, VertexAdjacent in enumerate(VertexMapping):
            if VertexAdjacent == IsAdjacentSign and Pointer != VertexIdx:
                AdjacentIdx.append(Pointer)

        return AdjacentIdx

    def _UnweakVertices(self):
        for Item in self.vertex:
            if Item is None:
                continue
            Item.weak = False
            ...
            for VertexIdx in range(len(self.vertex)):
                ...
                for Pointer, CurrentAdjacentIdx in enumerate(AdjacentIdxList):
                    ...
                    if len(IntersectedAdjacentIdx) > 0:
                        self.vertex[VertexIdx].NonWeak = True
                        self.vertex[CurrentAdjacentIdx].NonWeak = True
                        for IntersectedIdx in IntersectedAdjacentIdx:
                            self.vertex[IntersectedIdx].NonWeak = True
